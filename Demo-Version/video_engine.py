...
def video_tool(merchant_id, video_id, customer_id,lst=[]):
    ...
    if new_lst[0].endswith(('.mp4','.mov','.wmv','.avi')):
        ...
        paramterValue = pd.read_excel('parameterValue.xlsx')
        # con = od.connect(paramterValue[paramterValue['parameterName'] == 'connectionString']['value'].values[0])
        # # con.close()
        # con.autocommit = True

        connection = cx_Oracle.connect(paramterValue[paramterValue['parameterName'] == 'connectionString']['value'].values[0])
