...
def slide_show_zoomIn(path):
    ...
    print('Total Images Are:',len(no_of_images))
    ###################### END #######################################


    img_clip = []
    duration = 30//len(no_of_images)
    for image in natsorted(
            glob.glob(path + '/**.png')):  # bug here only handle png images find another way to iterate
        # print(image)
        x = ImageClip(image).resize(screensize).resize(resize_func).set_position(('right', 'left')).set_duration(
            duration).set_fps(25)
    ...
    return video.write_videofile('Test1.mp4')

#slide_show_zoomIn(r'C:\Users\user\Downloads\Towards Headlines\afghanistan')
